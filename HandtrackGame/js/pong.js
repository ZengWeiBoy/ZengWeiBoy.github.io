...
function startVideo() {
    handTrack.startVideo(video).then(function (status) {
        ...
        if (status) {
            updateNote.innerText = "正在追踪手部"
            isVideo = true
            runDetection()
        } else {
            updateNote.innerText = "摄像头未连接"
        }
    });
}
...
function toggleVideo() {
    if (!isVideo) {
        updateNote.innerText = "开启摄像头"
        startVideo();
    } else {
        updateNote.innerText = "关闭摄像头"
...
var scale_factor = 10  // 比例因子
var SPACE_WIDTH = windowWidth / scale_factor;  //空间宽度
var SPACE_HEIGHT = windowHeight / scale_factor; 


// Bead Details 小珠子细节
//var NUM_BEADS = 6
var NUM_BEADS = 10  // 一次掉几个
var BEAD_RESTITUTION = 0.7    //复原

// Paddle Details  灵敏度
//accelFactor = 0.042 * SPACE_WIDTH;
accelFactor = 0.05 * SPACE_WIDTH;
...
planck.testbed(function (testbed) {
    ...
    // Remove paddles that are no longer in frame. 移除不在框架内的拍子。
    ...
    world.on('pre-solve', function (contact) {
        ...
        if (apaddle || bpaddle) {
            // Paddle collided with something 桨与什么东西相撞
    ...
    // process mouse move and touch events 处理鼠标移动和触摸事件
    ...
    function addUI() {
        ...
        // Add mouse movement listener to move paddle 添加鼠标移动监听器以移动桨
        ...
        // Add keypress event listener to pause game 添加按键事件监听器暂停游戏
    ...
    // Generate Beeds falling from sky 产生从天上掉下来的芦苇
...
// 小球加减分控制，绿球的概率是0.2，红球的概率是0.2
            if (randVal > 0.9) {
                //   green ball, + 20
                beadColor.fill = '#32CD32'
                beadWidthFactor = 0.007  //小球的大小
                fd.userData.points = 20;
            } else if (randVal < 0.4) {
                //  Red Ball, - 10
                //beadWidthFactor = 0.007
				beadWidthFactor = 0.01
    ...
    // If the body is out of space bounds, wrap it to the other side  如果主体超出了空间界限，则将其包装到另一侧
    function wrap(body) {
...
function convertToRange(value, srcRange, dstRange) {
    // value is outside source range return  值在源范围之外返回
